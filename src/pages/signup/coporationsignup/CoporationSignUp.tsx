...
export default function CoporationSignUp() {
    ...
    const onAuthBusinessNumberChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
        const { value } = event.target;
  ...
  return (
    <div id='userauth-wrapper'>
        ...
        <div className='userauth-input-box'>
            ...
            <InputBox value={password} onChange={onPasswordChangeHandler} message={message} messageError={idMessageError} type='password' label='비밀번호' placeholder='비밀번호를 입력해주세요.'/>
            ...
            <InputBox value={authBusinessNumber} onChange={onAuthBusinessNumberChangeHandler} message={message} messageError type='text' label='사업자 번호 인증' placeholder='사업자번호를 입력해주세요.' buttonName='인증하기'/>
        </div>
